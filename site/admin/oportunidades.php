<!DOCTYPE html PUBLIC "-//W3C//DTD XHTML 1.0 Transitional//EN" "http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd">
    ...
    <title>Ambiente Restrito: ... Kffk oportunidades...</title>
...
if (isset($_POST['cadastrar'])){
    ...
    $sql = mysqli_query($link,"INSERT INTO oportunidade(`cod`, `nome`, `localizacao`,`descricao`,`publicar`, `log`) VALUES 
    ('','$nome','$localizacao','$descricao','$publicar', '$log')") or die ("Houve erro na gravação dos dados" . mysqli_error()); 

    if($sql) {
        echo "<script>alert('Oportunidade cadastrada com sucesso');</script>";
        echo "<meta http-equiv='refresh' content='0;URL=home.php?oportunidades&listar'> ";  
    } else {
  ...
  date_default_timezone_set('America/Sao_Paulo');
  $log = date('d/m/Y H:i:s', time());
  $sql = mysqli_query($link,"UPDATE oportunidade SET nome='$nome', localizacao='$localizacao', descricao='$descricao', publicar=$publicar WHERE cod=$cod ") or die ("Houve erro na gravação dos dados" . mysqli_error()); 
...
echo "<script>alert('Ocorreu algum erro. Tente novamente!');</script>";
  ...
  $cod = $_GET['cod'];
  $sql = mysqli_query($link,"SELECT * FROM oport_fotos WHERE cod_oport=$cod") or die("ERRO NO SQL".mysqli_error());
$row = mysqli_num_rows($sql);
if ($row == 0) {
echo "Não há fotos cadastradas para esta oportunidade";
} else {
while($row = mysqli_fetch_assoc($sql)){
?>
<a href="fotos/oportunidades/<?php echo $cod.'/'.$row['nome_foto']; ?>" target="_blank">
<img src="fotos/oportunidades/<?php echo $cod.'/'.$row['nome_thumb']; ?>" alt=" <?php echo $row['descricao']; ?>"  class="link" width="140"/></a>
<a class="link"  href="uploadfotos_oport.php?excluir&cod=<?php echo $row['cod']; ?>"><i class="fas fa-trash-alt"></i></a>
...
<form method="post" action="uploadfotos_oport.php?upload" enctype="multipart/form-data">
...
// ####################################################   VÍDEOS ########################################################
  ...
  $cod = $_GET['cod'];
  $sql = mysqli_query($link,"SELECT * FROM oport_videos WHERE cod_oport=$cod") or die("ERRO NO SQL".mysqli_error());
  $row = mysqli_num_rows($sql);
  if ($row == 0) {
  echo "Não há vídeos cadastrados para esta oportunidade";
  } else {
  while($row = mysqli_fetch_assoc($sql)){
    $cod_video = $row['cod'];
  $titulo = $row['nome'];
...
echo "</div><div>";
echo "<a href='oportunidades.php?editar_video&cod_video=$cod_video'><i class='fas fa-edit'></i></a>
<a href='oportunidades.php?excluir_video&cod_video=$cod_video'><i class='fas fa-trash-alt'></i></a>
...
<h2> Cadastrar vídeo</h2>
 <form action="oportunidades.php" method="post">
  ...
  $sql = mysqli_query($link,"INSERT INTO oport_videos (`cod`, `cod_oport`,`nome`, `descricao`,`id_youtube`) VALUES 
  ('','$cod_oport','$nome','$descricao','$id_youtube')") or die ("Houve erro na gravação dos dados" . mysqli_error()); 

  if($sql) {
      echo "<script>alert('Vídeo cadastrado com sucesso');</script>";
      echo "<meta http-equiv='refresh' content='0;URL=oportunidades.php?videos&cod=$cod_oport'> ";  
  } else {
    echo "<script>alert('Ocorreu algum erro. Tente novamente!');</script>";
      echo "<meta http-equiv='refresh' content='0;URL=oportunidades.php?videos&cod=$cod_oport'> "; 
  }
  ...
  $sql2 = mysqli_query($link,"SELECT * FROM oport_videos WHERE cod=$cod") or die ("Houve erro na gravação dos dados" . mysqli_error());  
  $dados = mysqli_fetch_array($sql2);
  $cod_oport = $dados['cod_oport'];

$sql = mysqli_query($link,"UPDATE oport_videos SET nome='$nome', descricao='$descricao', id_youtube='$id_youtube' WHERE cod='$cod' ") or die ("Houve erro na gravação dos dados" . mysqli_error()); 
...
echo "<meta http-equiv='refresh' content='0;URL=oportunidades.php?videos&cod=$cod_oport'> "; 
  ...
  $sql = mysqli_query($link,"SELECT * FROM oport_videos WHERE cod=$cod_video") or die ("Houve erro na gravação dos dados" . mysqli_error());  
...
<h2> Editar vídeo</h2>
 <form action="oportunidades.php" method="post">
  ...
  $sql = mysqli_query($link,"SELECT * FROM oport_videos WHERE cod=$cod_video") or die ("Houve erro na gravação dos dados" . mysqli_error());  
                    $dados = mysqli_fetch_array($sql);
                    $cod_oport = $dados['cod_oport'];
  $sql="DELETE FROM `oport_videos` WHERE `cod`='$cod_video'";
...
newdiv.innerHTML = "<a href=\"#\" onclick=\"removeEvent(\'"+divIdName+"\')\">&nbsp;[x]</a> <label >Título:</label><input class=\"form-control\" type=\"text\" name=\"titulo[]\" size=\"38\"><br> <label >Descri&ccedil;&atilde;o:</label><input class=\"form-control\" type=\"text\" name=\"desc[]\" size=\"38\"><br><label>Imagem:</label><input class=\"form-control\" type=\"file\" name=\"file[]\" size=\"44\"> <hr>";
